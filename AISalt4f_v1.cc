...
struct PLAYER_NAME : public Player {
    ...
    struct Unidad
    {
        ...
        Dir path;               //Camino a seguir por la unidad
    ...
    void camino_a_lo_mas_cerca(Unidad& u) {
        dijkstra_v2(u.unit.pos.i, u.unit.pos.j, u.path, 12);
    }

    void camino_al_danado(Unidad& u) {
        u.path = bfs_v2(u.id, 15);
    }
    ...
    bool inicio = true;         //Primera ronda
    bool cpu;                   //Me he pasado de CPU
    ...
    void update(bool init) {
        ...
        if (cpu) {
            ...
            for (int id : my_dwarves) {
                command(id, Dir(random(Bottom, LB)));
            }
            for (int id : my_wizards) {
                command(id, Dir(random(Bottom, LB)));
            }
        ...
        for (Unidad u : my_units) {
            if (u.path == None) u.path = Dir(random(Bottom, LB));
            command(u.id, u.next());
        }

    }

    Dir bfs_v2 (int id, int max_d) {
        ...
        {   
            ...
            if (pt.d > 0 and (pt.cell.id != -1) and (unit(pt.cell.id).player == my_id) and (unit(pt.cell.id).type == Dwarf)) {
                ...
                while (p[x][y] != -1) {
                    ...
                    x = origen.i;
                    y = origen.j;
                    path = calcular_direccion(origen, final);
    ...
    void dijkstra_v2(int i, int j, Dir& camino, int max_d) {

        int n = 60; //n = numero de filas
        int m = 60; //m = numero de columnas

        vector<vector<int> > d(n, vector<int> (m, INFINIT)); //d = vector de distancias
        d[i][j] = 0; //el origen tiene distancia 0
        vector<vector<int> > p(n, vector<int>(m,-1)); //p = vector de nodos previos
        vector<vector<bool> > visited(n, vector<bool>(m,false));
        priority_queue<Coord, vector<Coord>, comp > cola;
        cola.push(nueva_coord_bfs(i, j, 0));
        ...
        while (not cola.empty()) {
            ...
            if (d[pt.x][pt.y] < max_d) {
                ...
                if ((d[pt.x][pt.y] > 0) and (pt.cell.treasure or ((pt.cell.id != -1) and (unit(pt.cell.id).player != my_id)))) {
                    
                    if (distancia > d[pt.x][pt.y]) {
                        distancia = d[pt.x][pt.y];

                        /*cout << "Ronda " << round() << " unidad "<< cell(i,j).id << ": ";
                        if (pt.cell.treasure) cout << "Tesoro" << endl;
                        else cout << "Enemigo" << endl;*/

                        //if (unit(my_dwarves[0]).pos == Pos(i, j)) cout << "Tesoro en " << pt.x << " " << pt.y << " a " << distancia << " turnos" << endl;
                        ...
                        while (p[x][y] != -1) {
                            ...
                            x = origen.i;
                            y = origen.j;
                            camino = calcular_direccion(origen, final);
                        }
                ...
                if (pos_ok(nextpos.x, nextpos.y) and caminable2(nextpos) and not visited[nextpos.x][nextpos.y]) {
                    if (d[nextpos.x][nextpos.y] > (d[pt.x][pt.y] + nextpos.d)) {
                        ...
                        nextpos.d = d[nextpos.x][nextpos.y];
                        p[nextpos.x][nextpos.y] = Bottom;
                        cola.push(nextpos); // Top
                        visited[nextpos.x][nextpos.y] = true;
                    }
